...
public class AuthResource {
    ...
    public ResponseEntity<?> getAccesses() {
        User user = userService.getConnectedUser().orElseThrow(() -> new RuntimeException("no user found"));
        return ResponseEntity.ok(user.getAccesses().stream().map(a -> a.getType().name()).toList());
    }
}